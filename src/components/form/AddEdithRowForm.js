...
class AddEditRowForm extends React.Component {
    constructor(props) {
        super(props);
        this.state = {};
    ...
    render(){
       ...
       const fromgroup=this.props.formInput.map((formsinput,index)=>{
        if(formsinput.typeinput==='select'){
          return (
          <FormGroup key={formsinput.inputname} style={{float:"left",width:"45%",margin:"30px"}}>
          <Label for={formsinput.inputname}>{formsinput.label}</Label>
          {this.renderInputSelect(formsinput.inputname,formsinput.typeinput)}
        </FormGroup>)
        }else if(formsinput.typeinput==='text'||formsinput.typeinput==='number'||formsinput.typeinput==='date'){
            return(
        <FormGroup key={formsinput.inputname} style={{float:"left",width:"45%",margin:"30px"}}>
          <Label for={formsinput.inputname}>{formsinput.label}</Label>
          ...
          }else if(formsinput.typeinput==='time'){
            return (<FormGroup key={formsinput.inputname} style={{float:"left",width:"20%",margin:"30px"}}>
          <Label for={formsinput.inputname}>{formsinput.label}</Label>
            {this.renderInput(formsinput.inputname,formsinput.typeinput)}
        </FormGroup>)
          } else{
        ...
        return(
            <Form onSubmit={this.props.formaction ? this.submitFormEdit : this.submitFormAdd}>
                ...
                <FormGroup className="text-center" style={{marginTop:"20px"}}>
                  <Button className="btn btn-dark position-relative">Simpan</Button>
                </FormGroup>
            </Form>
        )
    }
}
export default AddEditRowForm